...
日常操作は `mcp_server.py` の 7 ツールを入口とし、`memory.py` の共通ロジックを CLI と共有する。Markdown Vault と local の二層を維持し、リモート HTTP サービスは導入していない。各端末・各クライアントが stdio プロセスを起動するため、アプリの接続設定・承認は残る。

保存先は `memory_config.py` で一度解決し、MCP の実行中は固定する。MCP は Vault の明示を必須とし、既存設定の破損、明示設定の不存在、不正パスで停止する。CLI フォールバックも同じ設定を使う。Windows と WSL は別ホストとして扱う。

日常保存はセッションを自動生成できるが、抽出では元の `session_id` とプロジェクトを保持する。保存成功後だけ抽出済みにする。CLI と MCP の両方に適用するストア境界の ID 検証、リンク拒否、プロセス間排他を追加した。ロックは同一端末の協調するプロセスに有効であり、Syncthing の端末間同時編集や人間の直接編集を同期しない。複数ファイルの原子的トランザクションではないため部分失敗時には現行記憶・event・observation を照合して回復する。

導入、登録設定の限定更新、互換 CLI とロック済み環境の違い、競合解消、未実機環境は [README](README.md) を正本とする。以下の SQLite、confidence による更新、FTS、将来 API 追加などの記述は歴史的設計であり、現行 API の操作手順として使わない。
...
- YAML frontmatter は `type`/`created`/`updated`（常時）と `tags`/`related`
  （非空の場合のみ）のフィールドで構成される（旧構造・削減理由、および
  `tags`/`related` を追加した経緯は「frontmatter の最小化（RDB 構造から
  の脱却）」章を参照）。
...
#### 追記: `tags`/`related` は最小化方針に反しない

上記の最小化方針の後に `tags`（タグ）と `related`（関連記憶の明示リンク）
を frontmatter へ追加した。これは方針の後退ではない。ここで削減した
`confidence`/`salience`/`sources` はいずれも「人間が読んでも意味を持たない
算出値・内部 provenance」だったのに対し、`tags`/`related` は Obsidian の
プロパティ表示・タグペイン・グラフビューがそのままネイティブに解釈できる
**人間によるキュレーション情報**であり、削減理由（RDB 列の残骸、アルゴリズ
ム的スコア、内部 ID 参照）のいずれにも当てはまらない。

- **`search` の複数タグは OR ではなく AND を採用した。** `search` は絞り
  込み操作であり、AND のほうが結果件数が予測しやすい。「関連しそうなもの
  を広く集める」という OR 的な用途は、後述の `related` ツールが別に担う
  ため、`search` 側に重複した緩い一致モードを持たせる必要がない。
- **`related` のランキングは、明示リンク 1 本につき 3.0（双方向で数える
  ため相互リンクは 6.0）、共有タグ 1 個につき 1.0 の単純な加算式にした。**
  ランキングの説明可能性を優先し、各ヒットに `score`/`matched_tags`/
  `link`（`outgoing`/`incoming`/`mutual`/`none`）を返すことで、呼び出した
  LLM が「なぜ関連と判定されたか」を検証してから結果を使えるようにした。
- **タグ希少度による重み付け（IDF 的な発想）は採用しなかった。** 現状
  数十件規模の個人 Vault では、頻出タグを自動的に軽く扱う恩恵より、
  スコアが直感的な合計値からずれて予測不能になるコストのほうが大きいと
  判断した。Vault の規模が数百〜数千件に育ち、頻出タグによる関連記憶の
  埋没が実害として観測された場合は、将来の拡張候補として再検討する。
- **タグ・関連のみの変更では `updated` を更新しない。** `updated` は
  `score_memory()` の recency ランキングの入力であり、本文の鮮度を表す
  シグナルである。バックフィル等でタグだけを一括付与した際に全件の
  `updated` が動いてしまうと、このシグナルの意味が失われる。同じ理由で
  `## 変更履歴` にも追記しない（DETAILS.md の「本文または type が変わった
  場合だけ更新」という既存規則の対象外という整理）。
