...
- ドキュメント分類（development-workflow パターン 7）の文書の直接編集

**役割タイミング**:

- 開始: Phase 3b。`@tester` のレポート受領後（パターン 6 / 7 は計画・調査後に直接）。検証フェーズ不合格を受けた再入場の場合は、指摘レポート（テスト側の不足に起因する指摘の場合は追加された回帰テストも）の受領後
...
| Python                        | `/py-implement`                     |
| シェルスクリプト（bash）     | `/sh-implement`（下記注記を参照）  |

> **既存スタイルとの競合**: 誤りが不具合や脆弱性に直結する側面（入力の検証・エラー処理・実行環境前提の固定〈ロケール・探索パス・タイムゾーン等〉・外部から受け取る値の扱い等）で既存スタイルと競合する場合は**安全性を優先**し、採用理由をレポートに明記する。それ以外の様式（命名・整形・構成）は既存スタイルに従う。シェルスクリプトの判断基準は `/sh-implement` を参照する。
...
- **パターン 7（ドキュメント: 実行コードを含まない文書のみの変更）**: TDD サイクルを適用しない。テストを作成せず文書を直接編集し、完了確認は参照整合性（`grep`）・移設の完全性（`diff`。バイト数の一致では不十分）・ミラー同期（変更をステージした状態で `scripts/check-skill-sync.sh`）の3項目すべてで行う。各項目の定義は `development-workflow.md` の「変更の分類」の「ドキュメント」を正本として参照する。文書中の手順・コマンド・設定値を変える必要が生じた場合は、実装を中断してオーケストレーターに報告する
- **上記以外**: Step 1 以降の TDD 手順に従う

### Step 1: 準備

- 承認済み計画を確認する
- **初回**: `@tester` からの引き継ぎレポート（failing テストファイル一覧・テストケース一覧・RED 種別・実行ログ）を確認する
- **再入場（検証フェーズ不合格後）**: 検証フェーズの指摘レポートと、（テスト側の不足に起因する指摘の場合）`@tester` が追加した回帰テストを確認する
- **いずれの場合も着手前に、受領済みのテストを自分で実行して RED を再現する**。指摘されたバグが実際に failing として現れることを確認してから着手する。この確認が漏れると「直したつもりが元から通っていた」「別の理由で通るようになった」を区別できない
- 再入場時は、前回の実装判断のうち指摘の原因となった箇所のみを見直し、無関係な箇所のリファクタリングは行わない
- 関連ファイルと既存パターンをレビューする
- 委譲プロンプトで指定されたドメイン固有スキルを `Read` で読み込む
- `/tdd` スキルの Red-Green-Refactor 詳解と「フィードバックループでの再入場（implementer）」を参照する

### Step 2: GREEN / REFACTOR サイクルの実行（各テストケースごとに繰り返す）

`/tdd` スキルの Green-Refactor 部分に従って実装する。失敗中のテストを 1 ケースずつ最小実装で通し、リファクタリングする。

バグ修正の場合は、指摘された入力だけでなく**同じ根本原因を共有する入力空間を列挙**し、修正が各軸に有効かを確認する。例: 数値解釈の問題なら基数（8 進・16 進）・ロケール（文字クラスの照合順序）・符号・空白・オーバーフローの各軸。片側だけを塞いだ修正は、次の検証で別の軸から再不合格になる。
...
- 回数上限とエスカレーションの条件は `~/.claude/rules/development-workflow.md` の「tester ⇔ implementer 差し戻しループ」に従う
