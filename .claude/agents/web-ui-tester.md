...
> **モックデータの本番整合性**:
>
> テスト用モックデータは本番 API が返すデータと同じ制約（件数・形式・範囲）で作成すること。
>
> - モックの件数は本番 API のパラメータから算出する（例: `past_hours=12 + forecast_hours=72` → 84件）
> - テストが本番データ量で通らない場合、テスト期待値が間違っているか、実装がその制約を扱えていない
> - `server.use()` でオーバーライドする場合も本番制約を維持する
> - バックエンドのエンドポイント定義を確認してデータ量を把握すること
> - 詳細は `/ui-debug` スキルの「モックデータの本番整合性」セクションを参照
