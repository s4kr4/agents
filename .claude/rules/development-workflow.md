...
→ 合格: ユーザーにコミット確認 / 不合格: 実装エージェントへフィードバック
...
| UI表示バグ修正 | 表示上の不具合の修正                                                                                           |
| 軽微           | スタイル値（色・余白・フォント・クラス名等）・表示文言・コメント・ドキュメント文字列のみで、ロジック（条件分岐・状態管理・データフロー・API・マークアップ構造）に一切触れない 1-2ファイルの変更 |

分類に迷う場合（特に「軽微」か「単純」か）は、より重い分類を適用する。

### フェーズ選択ガイド

| 分類                                   | 調査 | 計画 | 実装（RED→GREEN） | UI検証（※） | 検証 | パターン |
| -------------------------------------- | ---- | ---- | ----------------- | ----------- | ---- | -------- |
| 複雑（多ファイル、アーキテクチャ変更） | ✅    | ✅    | ✅                 | ✅           | ✅    | 1        |
| 中程度（複数ファイル、明確なパターン） | ✅    | ✅    | ✅                 | ✅           | ✅    | 2        |
| 単純（1-2ファイル、ロジックあり）      | ✅    | ❌    | ✅                 | ✅           | ✅    | 3, 4     |
